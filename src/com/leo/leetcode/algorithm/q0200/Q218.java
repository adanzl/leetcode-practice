...
public class Q218 {
    ...
    public List<List<Integer>> getSkyline(int[][] buildings) {
        ...
        TreeMap<Integer, Integer> hMap = new TreeMap<>((o1, o2) -> o2 - o1);
        ...
        while (!all.isEmpty()) {
            ...
            if (p[1] < 0) {
                // left point
                if (hMap.isEmpty() || hMap.firstKey() < -p[1]) {
                    ret.add(Arrays.asList(p[0], -p[1]));
                }
                hMap.put(-p[1], hMap.getOrDefault(-p[1], 0) + 1);
            } else {
                // right point
                if (hMap.get(p[1]) == 1) hMap.remove(p[1]);
                else hMap.put(p[1], hMap.get(p[1]) - 1);
                if (hMap.isEmpty() || hMap.firstKey() < p[1]) {
                    ret.add(Arrays.asList(p[0], hMap.isEmpty() ? 0 : hMap.firstKey()));
                }
            }
        }
        return ret;
