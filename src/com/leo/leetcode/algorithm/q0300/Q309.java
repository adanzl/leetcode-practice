...
//    }
    public int maxProfit(int[] prices) {
        ...
        int[] dp = new int[]{-prices[0], 0, 0};
        for (int i = 1; i < prices.length; i++) {
            int d0 = dp[0], d1 = dp[1], d2 = dp[2];
            dp[0] = Math.max(d2 - prices[i], d0);   // 手上持有股票的最大收益
            dp[1] = d0 + prices[i];                 // 手上不持有股票，并且处于冷冻期中的累计最大收益
            dp[2] = Math.max(d2, d1);               // 手上不持有股票，并且不在冷冻期中的累计最大收益
        }
