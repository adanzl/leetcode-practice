...
public class LCP09 {
    ...
    public int minJump1(int[] jump) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < size && !q.isEmpty(); i++) {
                ...
                int next = p + jump[p];
                q.offer(next);
                for (int j = Math.max(max + 1, p - 10000); j < p; j++) {
                    q.offer(j);
                }
                max = Math.max(max, p);
            }
    ...
    public int minJump(int[] jump) {
        ...
        for (int i = N - 1; i >= 0; i--) {
            int next = i + jump[i];
            dp[i] = next >= N ? 1 : dp[next] + 1;
            // 遍历当前位置更新后影响到的后面的位置，只需要更新到dp[j] >= dp[i]+1即可
            // 如果遍历到某dp[j]<dp[i]+1就不需要向右遍历了,因为 j 到 N 的值会被当前遍历到的dp[j]更新而不是dp[i]+1
