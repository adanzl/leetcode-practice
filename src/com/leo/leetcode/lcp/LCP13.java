 ...
 * 限制：
 * 1、1 <= maze.length <= 100
 * 2、1 <= maze[i].length <= 100
 * 3、maze[i].length == maze[j].length
 * 4、S 和 T 有且只有一个
 * 5、0 <= M的数量 <= 16
 * 6、0 <= O的数量 <= 40，题目保证当迷宫中存在 M 时，一定存在至少一个 O 。
...
public class LCP13 {
    ...
    public static void main(String[] args) {
        // 16
        System.out.println(new LCP13().minimalSteps(stringToStringArray("[\"S#O\", \"M..\", \"M.T\"]")));
        // -1
        System.out.println(new LCP13().minimalSteps(stringToStringArray("[\"S#O\", \"M.#\", \"M.T\"]")));
        // 17
        System.out.println(new LCP13().minimalSteps(stringToStringArray("[\"S#O\", \"M.T\", \"M..\"]")));
    }
